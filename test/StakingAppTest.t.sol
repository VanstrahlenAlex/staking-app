...
contract StakingAppTest is Test {
	...
	function testDepositTokensCorrectly() external {
		...
		uint256 elpasePeriodAfter = stakingApp.elapsePeriod(randomUser);

		assert(userBalanceAfter - userBalanceBefore == tokenAmount);
		assert(elapsePeriodBefore == 0);
		assert(elpasePeriodAfter == block.timestamp);
	...
	function testCanOnlyWithdraw0WhithoutDeposit() external {
		vm.startPrank(randomUser);
	...
	function testWithdrawTokensCorrectly() external {
		...
		assert(userBalanceAfter2 == userBalanceAfter2 + userBalanceInMapping);
